...
import { existsSync, writeFileSync, readFileSync, openSync, closeSync, mkdirSync } from "node:fs";
 ...
 * customised it. The generated path is recorded in Git's clone-local exclude.
 */
export function generateOverrideJson(projectRoot: string, configPath?: string, force = false, gitRoot?: string): void {
  const overridePath = join(projectRoot, ".pi", "devcontainer.override.json");
  mkdirSync(join(projectRoot, ".pi"), { recursive: true });
  excludeGeneratedArtifact(gitRoot, overridePath);
 ...
 */
export function startContainer(projectRoot: string, removeExisting = false, noCache = false, gitRoot?: string): void {
  const overridePath = join(projectRoot, ".pi", "devcontainer.override.json");
  const logPath = containerLogPath(projectRoot);
  mkdirSync(join(projectRoot, ".pi"), { recursive: true });
  excludeGeneratedArtifact(gitRoot, logPath);
