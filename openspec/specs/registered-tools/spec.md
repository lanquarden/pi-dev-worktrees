...
## Purpose
LLM-callable `worktree` and `devcontainer` tools registered via `pi.registerTool()` with action-dispatched parameters. Allows the LLM to chain worktree setup, devcontainer activation, and task work in a single prompt without external polling.
...
The extension SHALL extract the core logic for each operation into internal helper functions that are called from both the slash command handler and the tool `execute()` function.
